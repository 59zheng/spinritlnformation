 ...
 */
@Controller
@RequestMapping("/cure/tdct")
public class DbCureTdctController extends BaseController
{
    private String prefix = "cure/tdct";
    ...
    public List<DbCureTdct> list(DbCureTdct dbCureTdct)
    {
         ...
         *
         * */
        List<DbPatientMessageVo2> tms = TableListUtils.getListCurc("TDCT");
        List<DbCureTdct> list = new ArrayList<>();
        tms.forEach(item -> {
//            判断是否完成
            if (item.getDbPatientAssociated().getAssociatedId() != null) {
//                  绑定过
                DbCureTdct dbCureTdct1 = dbCureTdctService.selectDbCureTdctById(item.getDbPatientAssociated().getAssociatedId());
//                是否完成
                if (dbCureTdct1.getHowMany() == 0) {
//                    已完成
                    TableListUtils.updateResultOk(item.getDbPatientAssociated().getAssociatedId(), "TDCT", item.getDbPatientAssociated().getPatientId());
                ...
                DbCureTdct dbCureTms1 = new DbCureTdct();
...
//          数量
                ...
                List<DbCureTdct> dbCureTdcts = dbCureTdctService.selectDbCureTdctList(dbCureTms1);
                if (dbCureTdcts == null || dbCureTdcts.size() == 0) {
                    dbCureTms1.setExecutionTime(new Date());
                    dbCureTdctService.insertDbCureTdct(dbCureTms1);
                    list.add(dbCureTms1);
                }else {
                    list.addAll(dbCureTdcts);
                }
